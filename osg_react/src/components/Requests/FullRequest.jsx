...
const FullRequest = () => {
	...
	const getMyApplications = async () => {
		try {
			...
			const request = tunedApplications.find((app) => app.id === Number(id));
							...
							<div className="flex gap-6 px-4 pb-4 lg:flex-1 max-lg:items-center lg:gap-8 md:px-8 xl:px-12 max-lg:flex-col">
								<div className="flex flex-col flex-1 gap-6">
									...
									<div className="flex max-w-md gap-4 max-sm:flex-col">
										<button className="inline-block disabled:bg-neutral-400 select-none text-center w-full min-w-[8rem] hover:bg-neutral-600 px-8 py-2 rounded-full text-lg leading-none font-normal bg-black text-white border border-black">
											Прийняти
										</button>
										<button
											disabled
